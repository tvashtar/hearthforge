...
- FC-2 consumption: one RNG per campaign — `rng_seed` stored at creation; after every command the registry persists both `rng_draws` (audit count of dice drawn) and `rng_state` (the full serialized `random.Random` state). Reopening restores `rng_state` exactly (draw-count fast-forward is only a fallback for null state — it is not size-stable across mixed dice). All PC dice accept `player_value` (raw die total, flagged `player_supplied`); companion/monster dice never do. Hidden DM rolls set `gm_only`.
